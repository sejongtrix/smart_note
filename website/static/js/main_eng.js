...
function checkAllUploadsComplete() {
    if (isVideoUploaded && isPDFUploaded) {
        updateProgress(80);
        showNotification('All files have been uploaded. Redirecting to the post-processing page.', 'success');
        showLoading('Performing various post-processing tasks');

        // Redirect to /result and wait for it to complete
        fetch('/final_refine_eng')
            .then(response => {
                if (response.ok) {
                    return response.json(); // Wait for server response
                } else {
            ...
            .catch(error => {
                console.error(error);
                showNotification('An error occurred during result processing.', 'error');
            });
    }
}
...
function turntoresult() {
    updateProgress(100);
    showNotification('All post-processing is complete.', 'success');
    setTimeout(() => {
        window.location.href = '/result_eng';  // Flask에서 이 라우트를 처리해야 함
...
async function uploadVideoToServer(file) {
    showLoading('Uploading video and recognizing audio...');
    ...
    try {
        ...
        if (result.success) {
            ...
            updateProgress(10);
            showNotification('Video upload completed.', 'success');
            isVideoUploaded = true; 
            checkAllUploadsComplete(); 
        } else {
            showError('Video upload failed: ' + result.error);
        }
    } catch (error) {
        showError('An error occurred during video upload: ' + error.message);
    } finally {
        hideLoading();
    }
}
...
async function transfering_ai(file) {
    ...
    if (readyisready) {
        showNotification('Post-processing is complete.', 'success');
    }
    ...
    try {
        ...
        if (result.success) {
            transcriptData = result;
            updateProgress(80);
            showNotification('AI summarization completed.', 'success');
            isVideoUploaded = true; 
            checkAllUploadsComplete(); 
        } else {
            showError('AI summarization failed: ' + result.error);
        }
    } catch (error) {
        showError('An error occurred during AI summarization: ' + error.message);
    } finally {
        hideLoading();
    }
}
...
function updateProgress(percentage) {
    ...
    const messages = {
        10: 'Video file uploaded. mp3 conversion completed.',
        30: 'PDF file uploaded.',
        50: 'Image extraction completed.',
        80: 'Preprocessing started. Please wait for 10 minutes',
        100: 'All processing completed!'
    };
    
    progressText.textContent = messages[percentage] || `Processing... ${percentage}%`;
...
// Start progress bar processing state
...
// Stop progress bar processing state
...
function updateCurrentPage(audio, slider, timeDisplay) {
    ...
    audio.addEventListener('timeupdate', () => {
        ...
        .then(data => {
            ...
            img.onerror = () => console.error(`Failed to load image: ${imagePath}`); // Debug log for image load failure

            slideImageContainer.appendChild(img);
            ////// Not sure why this part doesn't work, so leaving it here
            ...
            if (currentPage !== null) {
                currentPageDisplay.textContent = 'Current page: 111';//`Current page: ${currentPage}`;
            } else {
...
function updateSlideImage(currentPage) {
    console.log(`Updating slide image: currentPage=${currentPage}`); // Debug log
...
function loadOpinion(page) {
    return fetch(`/get_opinion?page=${page}`)
        .then(response => response.json())
        .then(data => {
            console.log("Received data:", data);  // The response key should be present here
            return data.response;  // ✅ This key name must match the key in Flask's jsonify
        })
        .catch(error => {
            console.error("loadOpinion error:", error);
...
async function renderai(page) {
    ...
    // 비동기 데이터 받아오기
    const gemma_response = await loadOpinion(page);
    console.log("gemma_response :",gemma_response,"type",typeof gemma_response)
    // 필터링된 세그먼트 렌더링
    const segmentsDiv = document.createElement('div');
    segmentsDiv.className = 'segments';
